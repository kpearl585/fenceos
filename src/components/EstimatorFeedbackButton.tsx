...
/**
 * Estimator Feedback Button
 * Lightweight feedback capture for all estimators
 */
...
export default function EstimatorFeedbackButton({ designId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/feedback/estimator', {
        method: 'POST',
  ...
  if (!isOpen) {
    return (
      <button
        ...
        aria-label="Report Issue / Feedback"
        className="fixed bottom-6 left-6 bg-accent hover:bg-accent-light accent-glow text-white w-11 h-11 rounded-full shadow-lg flex items-center justify-center transition-colors duration-150 z-40"
      >
  ...
  return (
    <div className="fixed bottom-6 left-6 bg-surface-2 rounded-lg shadow-xl w-96 max-w-[calc(100vw-3rem)] border border-border z-50">
      {/* Header */}
