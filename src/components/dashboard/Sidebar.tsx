...
export function Sidebar({ items, orgName }: { items: NavItem[]; orgName: string }) {
  ...
  return (
    <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 bg-background text-white shadow-xl">
      {/* Brand */}
      <div className="flex items-center gap-3 h-16 px-5 border-b border-border">
        ...
        <span className="text-sm font-bold tracking-tight leading-tight">FenceEstimate<span className="text-accent-light">Pro</span></span>
      </div>

      {/* Org */}
      <div className="px-5 py-3 border-b border-border">
        <p className="text-xs text-muted uppercase tracking-wider font-medium">Organization</p>
        <p className="text-sm text-text truncate mt-0.5 font-medium">{orgName}</p>
      </div>
      ...
      <div className="px-4 py-4 border-t border-border">
        <p className="text-xs text-muted text-center">FenceEstimatePro v1.0</p>
      </div>
    </aside>
  );
}
