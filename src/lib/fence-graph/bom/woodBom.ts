...
export function generateWoodBom(
  ...
  if (gateSpecs.length > 0) {
    const gateCosts = calculateAllGateCosts(gateSpecs, "wood", prices, undefined, config);
    ...
    for (const gateCost of gateCosts.gates) {
      ...
      // Gate (single 4ft or double drive kit — distinct SKU per type)
      const gateKey = hw.gateSku;
      if (!gateSkuMap.has(gateKey)) {
        const gateDesc = hw.gateSku.endsWith("_DBL") ? "Wood Double Drive Gate" : "Wood Walk Gate (single)";
