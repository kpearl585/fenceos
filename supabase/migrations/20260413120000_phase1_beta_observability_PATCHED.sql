...
-- PATCHED: Fixed organizations reference + hardened RLS policies
-- ══════════════════════════════════════════════════════════════════
...
CREATE INDEX IF NOT EXISTS idx_phase1_events_org_created ON public.phase1_estimator_events(org_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_phase1_events_user_created ON public.phase1_estimator_events(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_phase1_events_type ON public.phase1_estimator_events(event_type);
CREATE INDEX IF NOT EXISTS idx_phase1_events_design ON public.phase1_estimator_events(design_id) WHERE design_id IS NOT NULL;

COMMENT ON TABLE public.phase1_estimator_events IS
  'Lightweight usage tracking for Phase 1 estimator beta';

-- RLS Policies (HARDENED with WITH CHECK)
ALTER TABLE public.phase1_estimator_events ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "phase1_events_org_policy" ON public.phase1_estimator_events;
...
CREATE INDEX IF NOT EXISTS idx_phase1_feedback_org_created ON public.phase1_estimator_feedback(org_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_phase1_feedback_unresolved ON public.phase1_estimator_feedback(org_id, resolved_at) WHERE resolved_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_phase1_feedback_design ON public.phase1_estimator_feedback(design_id) WHERE design_id IS NOT NULL;
...
DROP POLICY IF EXISTS "phase1_feedback_org_policy" ON public.phase1_estimator_feedback;
